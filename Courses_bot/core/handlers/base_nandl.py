...
async def start_bot(bot: Bot):
    ...
    await bot.send_message("1146756342", "Courses bot started.")

@base_router.shutdown()
async def stop_bot(bot:Bot):
    await bot.send_message("1146756342", "Courses bot stopped.")
...
async def about(callback:types.CallbackQuery, state: FSMContext):
    await state.clear()
    await callback.message.answer('''Понимание и усвоение теории может потребовать времени и усилий, особенно если материал сложен или незнаком. Чтобы правильно и эффективно вникать в теорию, рекомендую следовать следующим шагам:
...
Помните, что у каждого свой темп и стиль обучения. Главное — это находить радость и интерес в процессе погружения в новую теорию и не бояться задавать вопросы.''', reply_markup=inline.get_next_menu_kb())


@base_router.callback_query(BaseFSM.ABOUT, F.data == "Как работать с фреймворками")
async def about(callback:types.CallbackQuery, state: FSMContext):
    await state.clear()
    await callback.message.answer('''Фреймворк — это как рецепт или набор инструкций, который показывает, как делать что-то сложное шаг за шагом. Вместо того чтобы каждый раз придумывать, как готовить блюдо, вы следуете проверенному рецепту. Точно так же, вместо того чтобы создавать что-то с нуля в бизнесе или технологиях, вы можете использовать фреймворк как ваш рецепт или инструкцию. Вот несколько шагов и рекомендаций по правильной работе с фреймворком:
...
Ключ к успешной работе с фреймворком — это его понимание, гибкость в использовании и готовность к обучению и адаптации.''', reply_markup=inline.get_next_menu_kb())
    

@base_router.callback_query(BaseFSM.ABOUT, F.data == "Как применять знания на практике")
async def about(callback:types.CallbackQuery, state: FSMContext):
    await state.clear()
    await callback.message.answer('''Эффективное применение знаний на практике в работе - это ключевой элемент профессионального роста и успешной деятельности. Вот несколько советов, как это сделать:
...
В заключение, главное — это применять знания на практике, не бояться ошибок и учиться на них, а также постоянно стремиться к развитию и совершенствованию своих навыков.''', reply_markup=inline.get_next_menu_kb())


@base_router.callback_query(BaseFSM.ABOUT, F.data == "Как проводить рефлексию")
async def about(callback:types.CallbackQuery, state: FSMContext):
    await state.clear()
    await callback.message.answer('''Рефлексия — это процесс самонаблюдения и анализа собственных действий, чувств и мыслей. Рефлексивная практика может помочь вам лучше понимать себя, улучшить навыки принятия решений и улучшить отношения с другими. Вот несколько рекомендаций по работе с рефлексией:
...
Важно помнить, что рефлексия — это непрерывный процесс. Чем больше вы практикуетесь, тем легче будет анализировать свои действия и находить пути к личному и профессиональному росту.''', reply_markup=inline.get_next_menu_kb())
...
async def about(callback:types.CallbackQuery, state: FSMContext):
    await state.clear()
    await callback.message.answer("Ваши предложения, пожелания и вопросы вы можете написать в телеграмм @monoqle_support", reply_markup=inline.get_next_menu_kb())


@base_router.callback_query(BaseFSM.MENU, F.data == "Мои курсы")
async def about(callback:types.CallbackQuery, state: FSMContext):
    await state.clear()
    await callback.message.answer("Здесь будут ваши курсы", reply_markup=inline.get_next_menu_kb())